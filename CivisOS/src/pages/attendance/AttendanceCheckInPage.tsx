...
export function AttendanceCheckInPage() {
  ...
  return (
    <>
      <PageHeader title="Attendance Check-in" subtitle="Geo-fenced check-in and check-out" />
      {error ? <ErrorBanner message={error} /> : null}

      <Card>
        <h2>Quick actions</h2>
        <form className="toolbar" onSubmit={checkIn}>
          <Select value={geoFenceId} onChange={(e) => setGeoFenceId(e.target.value)} required aria-label="Geo-fence">
          ...
          <Table headers={['Date', 'Fence', 'Check-in', 'Check-out', 'Status']}>
            {myItems.map((a) => (
              <tr key={a.id}>
                ...
                <td>
                  <Badge tone={a.status === 'Rejected' ? 'danger' : a.status === 'CheckedOut' ? 'ok' : 'info'}>
                    {a.status}
                  </Badge>
                </td>
              </tr>
